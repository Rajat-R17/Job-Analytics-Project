...
function updateKPIs(total, jobs) {
  if (totalJobsEl) totalJobsEl.textContent = (total || 0).toLocaleString();
...
function exportToCSV() {
  if (!currentFilteredJobs || !currentFilteredJobs.length) {
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  setTimeout(initializeExplorer, 1);
} else {
  window.addEventListener("DOMContentLoaded", initializeExplorer);
}