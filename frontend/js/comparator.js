...
function loadCompetitivenessRadar(nameA, nameB, resA, resB) {
  ...
  const totalA = resA.total || resA.data.length;
  const totalB = resB.total || resB.data.length;

  const avgA = getAverageSalary(resA.data);
  const avgB = getAverageSalary(resB.data);

  // Helper for skill diversity count
  ...
  const skillDivA = getSkillCount(resA.data);
  const skillDivB = getSkillCount(resB.data);

  // Normalize metrics to 0-100 relative scores
...
async function loadComparison() {
  ...
  const nameA = cityA.value;
  const nameB = cityB.value;
  ...
  const topSkillA = getTopSkill(resA.data);
  const topSkillB = getTopSkill(resB.data);

  const topCompA = getTopCompany(resA.data);
  const topCompB = getTopCompany(resB.data);
  ...
  const bandsA = getSalaryBands(resA.data);
  const bandsB = getSalaryBands(resB.data);
